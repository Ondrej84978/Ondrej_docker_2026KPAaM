...
if __name__=="__main__":
    ...
    pb.setJointMotorControl2(robot_id, joint1_idx, controlMode=pb.POSITION_CONTROL, targetPosition = theta2)
    ...
    for t in range(num_steps):
        pb.addUserDebugLine(link1_position, link1_position + T_link1[:3, 0] * axis_lenght, [1, 0, 0], lineWidth=3, lifeTime=1.0 )
        pb.addUserDebugLine(link1_position, link1_position+ T_link1[:3, 1] * axis_lenght, [0, 1, 0], lineWidth=3, lifeTime=1.0 )
        pb.addUserDebugLine(link1_position, link1_position + T_link1[:3, 2] * axis_lenght, [0, 0, 1], lineWidth=3, lifeTime=1.0 )
        
        pb.addUserDebugLine(link2_position, link2_position + T_link2[:3, 0] * axis_lenght, [1, 0, 0], lineWidth=3, lifeTime=1.0 )
        pb.addUserDebugLine(link2_position, link2_position+ T_link2[:3, 1] * axis_lenght, [0, 1, 0], lineWidth=3, lifeTime=1.0 )
        pb.addUserDebugLine(link2_position, link2_position + T_link2[:3, 2] * axis_lenght, [0, 0, 1], lineWidth=3, lifeTime=1.0 )
        pb.stepSimulation()
        time.sleep(1/240)
    pb.disconnect()